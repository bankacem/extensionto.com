...
router.put('/articles/:slug', async (request: Request, env: Env) => {
    try {
        const { slug } = request.params;
        const data = await request.json();
        const { title, content_html, meta_description, keywords, featured_image, status, category, scheduled_at } = data;
        ...
        const { success } = await env.DB.prepare(
            'UPDATE articles SET title = ?, content_html = ?, meta_description = ?, keywords = ?, featured_image = ?, status = ?, category = ?, scheduled_at = ? WHERE slug = ?'
        )
            .bind(title, content_html, meta_description, keywords, featured_image, status, category, scheduled_at, slug)
            .run();
        ...
        return new Response('Failed to update article', { status: 500, headers: corsHeaders });
    } catch (error: any) {
        return new Response(error.message, { status: 500, headers: corsHeaders });
    }
});
...
router.delete('/articles/:slug', async (request: Request, env: Env) => {
    try {
        const { slug } = request.params;
        const { success } = await env.DB.prepare('DELETE FROM articles WHERE slug = ?').bind(slug).run();
        ...
        return new Response('Failed to delete article', { status: 500, headers: corsHeaders });
    } catch (error: any) {
        return new Response(error.message, { status: 500, headers: corsHeaders });
    }
});
